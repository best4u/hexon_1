...
require_once (plugin_dir_path(__FILE__)."ParallelGet.php");
...
function url_param()
{
    ...
    $GLOBALS['global_url_param'] = [
        ...
        'url_slug' => isset($url_param[2]) ? $url_param[2] : null,
    ];
...
function occasions_list_overview($atts = array())
{
    ...
    if($first_time) {
        ...
        if ($url_param['id']) {
            ...
            $parallel_curl = new ParallelGet($carsService->getOverviewUrls($filter, $count));
            $result = $parallel_curl->get();
            $cars = @$result['cars'];
